...
class OceanEmbedSettings(BaseSettings):
    """OceanEmbed-specific configuration settings.

    Controls storage paths, Zarr pointer re-resolution behaviour, and
    quality-gate thresholds.  All values are read from environment variables
    (or the .env file) so that no paths are ever hardcoded in source.
    """

    # Path to the published Zarr root on the host / container.
    # The directory must contain week=YYYY-Www/ subdirs and a 'latest' symlink.
    PUBLISHED_ZARR_ROOT: str = config("PUBLISHED_ZARR_ROOT", default="/data/published")

    # How often (seconds) the API re-checks the published/latest symlink.
    # A new week is published once a week, so 30s is well within budget.
    ZARR_POINTER_RECHECK_SECONDS: int = config(
        "ZARR_POINTER_RECHECK_SECONDS", default=30, cast=int
    )

    # Path to the frozen model registry on the host / container.
    # The worker mounts this read-only; it never writes here.
    MODEL_REGISTRY_PATH: str = config(
        "MODEL_REGISTRY_PATH", default="/model-registry"
    )

    # Z-score threshold for the per-channel quality gate.
    # Cells with |z| > this value are flagged as outliers.
    QUALITY_GATE_ZSCORE_THRESHOLD: float = config(
        "QUALITY_GATE_ZSCORE_THRESHOLD", default=4.0, cast=float
    )

    # Maximum fraction of outlier cells before the gate fails a channel.
    QUALITY_GATE_ZSCORE_MAX_FRACTION: float = config(
        "QUALITY_GATE_ZSCORE_MAX_FRACTION", default=0.05, cast=float
    )
...
class Settings(
    ...
    """Main settings class combining all setting categories."""
